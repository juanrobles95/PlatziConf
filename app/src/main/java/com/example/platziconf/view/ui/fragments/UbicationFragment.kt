...
class UbicationFragment : Fragment() , OnMapReadyCallback, GoogleMap.OnMarkerClickListener{
    ...
    override fun onMapReady(googleMap: GoogleMap?) {
        ...
        val zoom = 16f
        val centerMap =  com.google.android.gms.maps.model.LatLng(ubicacion.latitude, ubicacion.longitude)
        googleMap?.animateCamera(CameraUpdateFactory.newLatLngZoom(centerMap,zoom))

        val centerMark = centerMap
        val markerOption = MarkerOptions()
        markerOption.position(centerMark)
        markerOption.title("Platzi Conf 2019")

        val bitmapDraw = context?.applicationContext?.let { ContextCompat.getDrawable(it,R.drawable.logo_platzi) } as BitmapDrawable
        val smallMarker = Bitmap.createScaledBitmap(bitmapDraw.bitmap,90,90,false)
